...
- **The voice-guardian pack is a build product too.** An optional
  ...
  the source versions it was built from, and never edited directly. CI
  (`.github/workflows/pack-freshness-check.yml`) fails any PR that changes a
  voice-relevant file without regenerating the pack; kits without a pack skip the check.
  The pack is exempt from the retired-language grep (it must quote retired phrases
  verbatim to enforce them). The last mile — re-uploading the regenerated pack into the
  custom GPT or skill — is a human step; the pack's stamp is how you verify a deployed
  copy is current.
